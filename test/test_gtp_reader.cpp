...
TEST_CASE("GP4 overstated beat count keeps measure alignment", "[gtp][integration]") {
    const auto file_data = load_test_file("overstated_beats.gp4");
    REQUIRE_FALSE(file_data.empty());
    REQUIRE(is_gtp_file(file_data));
    ...
    const Song &song = result.value();
    REQUIRE(song.name() == "Overstated Beats GP4");
    REQUIRE(song.artist() == "Fixture");
    REQUIRE(song.tempo_bpm == 120);
    REQUIRE(song.track_count() == 1);
    REQUIRE(song.measure_count() == 2);
    REQUIRE(song.tracks.front().measures.size() == 2);
    REQUIRE(song.tracks.front().measures[0].beats.size() == 1);
